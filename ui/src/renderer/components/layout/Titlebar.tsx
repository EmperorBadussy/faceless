...
export function Titlebar() {
  ...
  const handleMaximize = useCallback(async () => {
    window.phantomFace.windowMaximize()
    const isMax = await window.phantomFace.windowIsMaximized()
    setMaximized(isMax)
  }, [])
  ...
  return (
    <div className="drag-region fixed top-0 left-0 right-0 z-50 h-[36px] flex items-center border-b border-phantom-800 bg-phantom-950/90 backdrop-blur-md px-4">
      ...
      <div className="no-drag flex items-center">
        <button
          onClick={() => window.phantomFace.windowMinimize()}
        ...
        <button
          onClick={() => window.phantomFace.windowClose()}
